...
| quality | `tpm.quality.run_quality(ws, settings, ctx)` | dataset, schema, signals, `rules.json` | `checks.jsonl` (CheckResult), `trust.jsonl` (TrustVerdict), `batches.json` |
| detect | `tpm.detect.run_detect(ws, settings, ctx)` | dataset, schema, signals, relations, trust | `scores.parquet` (per-row scores + contributions), `flags.jsonl` (Flag), `patterns.json` (FaultPattern), `baseline.json`, `detect_meta.json`, `evaluation.json` (only if label columns exist) |
