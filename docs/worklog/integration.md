...
## 2026-09-19 (afternoon) — Round 3 complete; root cause of the "dead UI" found and fixed
- DEAD SERVER ROOT CAUSE (py-spy dump of the hung process): 16 routes were `async def` but called blocking code on the event loop (post_chat -> agent.chat -> LocalIndex.build -> httpx.post to Ollama). One model-backed request froze the server for every client; two demo servers died this way (I first blamed big uploads: wrong). Fix: `_offload` decorator in tpm/api/server.py (body cached on the server loop, handler coroutine run in a worker thread), pooled httpx client + 127.0.0.1 instead of localhost (Windows pays ~2 s IPv6 fallback per new connection), search-index embeddings time-boxed (<= 600 items, 20 s) with TF-IDF fallback. Verified live: 40 requests <= 0.04 s while an 8.5 s chat ran.
- Hardening: run_id validated where a workspace is created from a request; runs whose owning process is gone are marked failed/"interrupted" (pid stored in meta.json at run start; psutil check incl. pid reuse).
- UI subagent: streamed upload (tpm/api/upload.py, 8 MB blocks, 630 MB at 211 MB/s with a 70 MB memory peak) + progress screen, pages unlock with stages, trust-by-batch reasons with row ranges, runs list last 5, suspicious-rows list + point flags in Monitor.
- Export subagent: tpm/report/pdf.py (reportlab), pptx_export.py (python-pptx), export_common.py; routes /report.pdf|.pptx, CLI --format, export zip, e-mail attachments. Checked on swat_points EN/FI: 48-page PDF ~1 s, 12-slide deck with 7 native charts.
- Chat: "Ask why" on a suspicious row loads that row's entry (tpm/llm/agent.py load_context).
- Demo: frozen worktree ../norrin_demo served on :8000 with TPM_WORKSPACE pointing at the main workspace, so in-progress edits cannot break a demo. Refresh with `git -C ../norrin_demo checkout --detach <commit>` and restart.
Open: no route to stop a running analysis (DELETE answers 409 while a job runs); assessor_upload/stream_push still read an uploaded file into memory; print-to-PDF of the HTML report unverified (native PDF export exists now); tpm/assessor/actions.py still emits "Yes:/No:" rationales (consumers strip them).
