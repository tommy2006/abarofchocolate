...
| A critique that always agrees | supported 3,856 of 3,857 | argues four alternatives from the evidence (data problem, process change, saturated actuator, single broken sensor) and weakens a diagnosis when the data under it failed the trust check: 2,692 of 3,866 weakened on this file, every disagreement logged |
...
| Flat attribution | 9 %, 9 %, 4 % ... as a precise list | when no signal carries 20 % of the deviation, the headline says "no single signal dominates: about 11 signals of cluster C02" instead of a precise-looking list; a lag reference is named when the onset order can be measured, with upstream / downstream wording |
| Unnamed patterns | "PATTERN-A (unnamed)" | named from the plant's list of known failure types when enough of its sensors lead, e.g. "possibly Fault 4: reactor cooling water inlet temperature step", otherwise "cannot name" with the closest candidate |
| Data quality | 20 frozen signals = 20 findings; trust verdicts that never failed | one grouped finding for signals frozen together, duplicates and frozen blocks now make a batch untrusted (10 of 10 on this file, mainly duplicated rows), plausible ranges per signal, and timeliness says "not testable" instead of a fake pass |
