...
export async function render(main, params = {}) {
  ...
  const checksOfBatch = (bid) => {
    const kinds = new Map(); const groups = new Map(); let passed = 0; let untestable = 0;
    for (const c of checksBy.get(bid) || []) {
      const cat = catOf(c); const gr = groups.get(cat) || { status: 'pass', kinds: new Map(), nPass: 0, nNt: 0 }; groups.set(cat, gr);
      if (isOk(c)) { passed++; gr.nPass++; continue; }
      // not testable (e.g. timeliness without a time column): its own grey state, never a pass and never a problem
      if (isNT(c)) { untestable++; gr.nNt++; continue; }
    ...
    return { kinds: [...kinds.values()].sort((a, b) => (b.status === 'fail') - (a.status === 'fail') || b.n - a.n), passed, groups, untestable };
  };
  ...
  const checkChips = (bid) => {
    ...
    return el('div', { class: 'dq-chips dq-questions' },
      cats.map((cat) => {
        ...
        const nt = g.status === 'pass' && !g.nPass && g.nNt > 0;  // every check of this question could not run on this data
        return el('span', { class: 'dq-chip ' + (nt ? 'nt' : g.status === 'pass' ? 'ok' : g.status), title: nt ? `${title}. ${t('dq.notTestableHelp')}` : title }, el('span', { class: 'q', text: vt('dq.group.' + cat) }), ' ',
          nt ? el('b', { text: t('dq.not_testable') }) : g.status === 'pass' ? el('b', { text: '✓' }) : [el('b', { text: g.status === 'fail' ? '✕ ' : '! ' }), [...g.kinds.values()].map((k) => k.label + (k.n > 1 ? ` ×${k.n}` : '')).join(' · ')]);
      }),
