...
export async function render(main, params = {}) {
  ...
  function show(d) {
    ...
    detail.append(el('div', { class: 'row small muted', style: { marginTop: '4px' } }, el('span', { text: d.id }), el('span', {}, `${t('common.group')} `, d.group_id !== null && d.group_id !== undefined ? refLink('group', String(d.group_id)) : '–'), d.pattern_id ? refLink('pattern', d.pattern_id) : null, d.narrative_source ? el('span', { text: `${t('common.source')}: ${d.narrative_source}` }) : null));
    // plain verdict line: what, how sure, how serious
    detail.append(el('p', { class: 'verdictline' }, el('b', {}, t('diag.howSure')), ' ', confWords(d.confidence), ' · ', el('b', {}, t('common.severity')), ' ', sevWords(sevOf(d))));
    // summary as prose
    detail.append(el('h4', { class: 'small muted', style: { marginTop: '12px' }, text: t('diag.summary') }), prose(d.summary || d.fault_type));
    // cause reasoning
    const whyKey = 'diag.why.' + (d.cause_class || 'unknown');
    detail.append(el('h4', { class: 'small muted', text: t('diag.causeWhy') }), el('p', { class: 'prose' }, causeChip(d.cause_class), ' ', t(whyKey) === whyKey ? t('diag.why.unknown') : t(whyKey)));
    // ranked signals
    if ((d.ranked_signals || []).length) {
      detail.append(el('h4', { class: 'small muted', text: t('diag.signals') }));
      detail.append(el('div', {}, d.ranked_signals.map((s) => el('div', { class: 'meter', title: cleanText(s.explanation || '') }, el('span', {}, refLink('signal', s.signal), ' ', el('span', { class: 'dim small', text: dirWord(s.direction) })), el('span', { class: 'bar' }, el('i', { style: { width: Math.min(100, s.contribution * 100) + '%' } })), el('span', { class: 'right', text: fmt.pct(s.contribution) })))));
      ...
      if (expl.length) detail.append(el('ul', { class: 'list small prose' }, expl.map((x) => el('li', {}, linkifyRefs(x.text), x.ev.length ? [' ', evidenceButton(x.ev, { label: t('common.evidence') })] : null))));
    }
    // propagation chain
    if ((d.propagation || []).length) {
      ...
      detail.append(proseList(sentences, { cls: 'list small prose' }));
    }
    // steps as numbered full sentences
    if ((d.steps || []).length) detail.append(el('h4', { class: 'small muted', text: t('diag.steps') }), proseList(d.steps, { ordered: true }));
    // critique
    if (d.critique) {
      const c = d.critique;
    ...
    if (fs.length) detail.append(el('h4', { class: 'small muted', style: { marginTop: '12px' }, text: t('diag.flags') }), el('ul', { class: 'list plain flaglist' }, fs.map((f) => el('li', { class: 'row' }, refLink('flag', f.id), f.missing ? null : [kindChip(f.kind), sev(f.severity), el('span', { class: 'small muted flagstmt' }, linkifyRefs(cleanText(f.statement || ''))), el('button', { class: 'btn btn-sm btn-quiet', type: 'button', onClick: () => openChat(flagContext(f)) }, t('common.ask'))]))));
    detail.append(el('div', { style: { marginTop: '12px' } }, evidencePanel(d.evidence_ids || [])));
  }
  renderList();
