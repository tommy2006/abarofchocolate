...
const STYLE = 'margin:14px 0 18px;padding:14px 18px;border-left:4px solid var(--accent, #2bb5a0);background:var(--bg-2, rgba(127,127,127,.08));border-radius:0 8px 8px 0;max-width:100%;overflow-wrap:anywhere';
...
export async function plainBox(view) {
  ...
  const box = el('section', { class: 'plain', style: STYLE },
    el('div', { class: 'plain-head', style: 'display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap' }, el('h3', { text: t('plain.title'), style: 'margin:0' }), el('button', { class: 'btn btn-sm', type: 'button', onClick: async (ev) => {
